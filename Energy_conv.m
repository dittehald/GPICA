...
Wone = reshape(Wall_withlags, d*prs.m*(prs.lags +1), prs.nsamples );
...
%%
Zlagall =  lags_shift(Z,prs.lags); % creates shifted version of Z
Residual = zeros(prs.m,1);
EnergyX = norm(X_0mean,'fro')^2;
% e = repmat(X_0mean,[1 1 prs.m]);
for m=1:prs.m
    e = X_0mean;
    for la=1:prs.lags+1
        Zlag = Zlagall(:,:,la);
        Wlag =  squeeze(Wall_withlags(:,:,la));
        e = e - squeeze(Wlag(:,m)*squeeze(Zlag(m,:)));
    end
    Residual(m) =norm(e,'fro')^2;
end
...
% % % % % % 
% % % % % % Word = Wall_withlags(:, ordr5(K:-1:1),:,:); % second time?
